 ...
 *
 * <p><b>三类请求各算各的配额，互不占用：</b>
 * <table>
 *   <tr><td>POST /voucher-order/seckill/{id}</td><td>下单</td><td>严格（5 次/秒）</td></tr>
 *   <tr><td>GET /voucher-order/seckill/result/{id}</td><td>查落库</td><td>宽松但要封顶</td></tr>
 *   <tr><td>GET /voucher-order/pay/result/{id}</td><td>查支付</td><td>宽松但要封顶</td></tr>
 * </table>
 *
 * <p>为什么要拆开：共用一个 key 的话，前端为了拿到结果多查几次，就把自己的
 * 下单额度烧光了——真正想下单时反而被 429。两者性质不同，配额也该不同。
 *
 * <p><b>限流的真正目的是防穿透，不是防轮询洪峰。</b>
 * 轮询规模被库存数封顶：只有秒杀成功（或降级拿到 orderId）的人才查得了，
...
public class SlidingWindowInterceptor implements HandlerInterceptor {
    ...
    /** 支付结果查询的 URI 片段 */
    private static final String URI_PAY_RESULT = "/pay/result/";
    ...
    // ---- 支付结果查询：宽松但要封顶 ----
    @Value("${seckill.rate-limit.sliding-window.pay-window-ms:1000}")
    private long payWindowMs;

    @Value("${seckill.rate-limit.sliding-window.pay-max-requests:10}")
    private int payMaxRequests;
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
        ...
        int max;
        long window;
        // 只有「下单」这一个动作计入秒杀请求数。结果查询与支付查询是下单派生出来的
        // 后续轮询，算进分母会让「被限流比例」失真（看起来限流很轻）。
        // 默认分支按下单处理：将来若挂了新 path 忘了配规则，至少还有严格限流兜着。
        boolean isSeckillSubmit = !uri.contains(URI_PAY_RESULT) && !uri.contains(URI_SECKILL_RESULT);

        if (uri.contains(URI_PAY_RESULT)) {
            key = RateLimitConstants.SLIDING_WINDOW_ORDER_PAY_KEY + userId;
            max = payMaxRequests;
            window = payWindowMs;
        } else if (uri.contains(URI_SECKILL_RESULT)) {
