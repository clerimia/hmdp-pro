...
public class RocketMQProducer {
    ...
    /**
     * 发送订单创建事务消息：先半消息，再在 TransactionListener 中执行 Lua，成功则 COMMIT。
     * MQ 侧唯一的业务消息入口——对账补单不走 MQ（同步直调 createOrderFromMQ，
     * 对账是 MQ 故障的兜底，不能反过来依赖 MQ），本类不再有普通消息发送方法。
     *
     * <p>P2 容错：{@code mqBreaker} 统计发送失败率（50%/窗口 20/半开 10s）。熔断打开后新请求
     * 直接被 fallback 语义化拒绝——半消息不再发、本地事务（Lua）不会执行，Redis/DB 一个都不碰，
     * 这就是「MQ 挂时领券快速失败，不打 DB」。
     *
