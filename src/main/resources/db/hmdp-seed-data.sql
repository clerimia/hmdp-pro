...
-- ----------------------------
-- 7. 秒杀库存（账本：stock = initial_stock - 已领取数）
--    券 10 进行中：200 - 3 = 197
--    券 11 未开始：100 - 0 = 100
--    券 12 已结束：100 - 4 = 96
-- ----------------------------
...
--    券 10 进行中活动 3 单（2 领取未使用 / 1 已使用）；券 12 已结束活动 4 单（1 领取未使用 / 3 已使用）；
...
-- 券 12（已结束活动）洪峰：worker=2，同一秒 seq 0/1/2/3；3 条核销落在活动结束后数小时内
