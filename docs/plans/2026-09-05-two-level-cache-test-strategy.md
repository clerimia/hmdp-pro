...
#### A4 · 逻辑过期返回旧值 + 异步重建且旧值不进入 L1（慢）
- **前置**：DB 把 shop 1 改名为「新名」；直连 Redis 塞 `{"data":{...,"name":"旧名"},"expireTime":<过去时刻>}`
- **步骤**：GET → 记耗时 → `wait_until` Redis 内 `data.name == "新名"`（timeout 10s）→ 立即再 GET
- **断言**：
  1. 第一次响应 `name == "旧名"`（逻辑过期同步返回旧值，不阻塞）
  2. 异步重建后 Redis 内 `data.name == "新名"`，且 `rebuild{result=ok}` 增量 >= 1
  3. **重建完成后立即再 GET，返回「新名」**（证明逻辑过期旧值未进入普通 L1）
- **标记**：`slow`（31s）

> **第 3 条是这个用例的价值所在。**逻辑过期值只服务当前请求；若放入普通 30 秒 L1，Redis 已重建后仍会被旧 L1 挡住，删除场景下还可能发生异步线程清 L1 后请求线程又写回幽灵旧值的竞态。JVM in-flight 标记负责提交前去重，Redisson 锁负责跨实例去重。
